...
/* ─────────────────────────────────────────────
   DOME GALLERY – 3D hemisphere image gallery
   Based on reactbits.dev/components/dome-gallery
   Adapted for Next.js 14 + styled-components
   ───────────────────────────────────────────── */
...
/* ── Global CSS (injected when component mounts) ── */

const DomeGalleryStyles = createGlobalStyle`
  .dg-scroll-lock {
  ...
  .overlay {
    background-image: radial-gradient(
      ...
      var(--overlay-blur-color, #F0ECF5) 100%
    );
  }
  ...
  .overlay--blur {
    -webkit-mask-image: radial-gradient(
      ...
      var(--overlay-blur-color, #F0ECF5) 90%
    );
  ...
  useGesture(
    {
      ...
      onDrag: ({ event, last, velocity = [0, 0], direction = [0, 0], movement }) => {
        ...
        if (last) {
          ...
          let vx2 = vMagX * dirX;
          let vy2 = vMagY * dirY;
          if (Math.abs(vx2) < 0.001 && Math.abs(vy2) < 0.001 && Array.isArray(movement)) {
            const [mx, my] = movement;
            vx2 = clamp((mx / dragSensitivity) * 0.02, -1.2, 1.2);
            vy2 = clamp((my / dragSensitivity) * 0.02, -1.2, 1.2);
          }
          if (Math.abs(vx2) > 0.005 || Math.abs(vy2) > 0.005) startInertia(vx2, vy2);
